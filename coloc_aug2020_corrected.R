...
#select cruise
cruise<-filter(cruise_data, cruise=='7')
...
tiff( "./figures/Collocation_manuscript/revisions_conscipractice/colloc_all_spp_by_cruise_CORRECTED.tif",width = 10, height = 8, units = 'in', res = 300)
par(mfrow=c(2,6))
...
#set colors for transparent circles for counts
col2rgb("black")
mycol <- rgb(0, 0, 0, max = 255,alpha=150, names = "newblack")
colcode<-mycol

tiff( "./figures/Collocation_manuscript/revisions_conscipractice/w_totals_CORRECTED.tif", width = 10, height = 8, units = 'in', res = 300)
...
plot(sbnms, add=T) 
points(cruise$lon, cruise$lat,cex=cruise$w_sym_size, pch=16,col=mycol)
...
tiff( "./figures/Collocation_manuscript/revisions_conscipractice/totalGIC_CORRECTED.tif",width = 7, height = 10, units = 'in', res = 300) #create a tiff device, sized and 300 dpi res
